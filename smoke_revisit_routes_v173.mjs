  ...
  try {
    ...
    const fresh = await evalByValue(client, `(() => {
      ...
      return {
        ...
        hasPreviewOnlySafety: text.includes('Locked preview. No route access.') || text.includes('No route access'),
        hasRouteAccessUnavailable: text.includes('No route access'),
    ...
    record(
      ...
      /Locked previews only/.test(malformed.text) &&
      /Need:/.test(malformed.text) &&
    ...
    record('Trophy Echo exposes preview copy without becoming playable', Array.isArray(mutationCheck.previews) && mutationCheck.previews.every(preview => preview.locked === true && preview.playable === false) && !!mutationCheck.previews.find(preview => preview.key === 'trophy_echo_route' && preview.previewState === 'preview' && preview.previewLabel === 'Future Unlock Preview' && /future boss history/i.test(preview.previewReason || '') && preview.previewRequirement === 'Build more boss history.' && /No route access/i.test(preview.previewSafety || '')) && !!String(mutationCheck.routeSlotText || '').includes('Future Unlock Preview') && !!String(mutationCheck.routeSlotText || '').includes('Still locked') && !!String(mutationCheck.routeSlotText || '').includes('Safety:'), JSON.stringify({ trophyPreview: mutationCheck.previews?.find(preview => preview.key === 'trophy_echo_route'), previewSummary: mutationCheck.previewSummary, routeSlotText: String(mutationCheck.routeSlotText || '').slice(0, 500) }));
    ...
    record('Earlier Dungeon Revisit shows Trophy Echo planning copy without controls', /Optional side routes from existing records/i.test(trophyPlanCheck.routeSlotText || '') && /Routes stay locked until their records are ready/i.test(trophyPlanCheck.routeSlotText || '') && /Locked previews only/i.test(trophyPlanCheck.routeSlotText || '') && /Gate diagnostics stay read-only/i.test(trophyPlanCheck.routeSlotText || '') && /Trophy Echo Rule Planning/i.test(trophyPlanCheck.routeSlotText || '') && /Planning only/i.test(trophyPlanCheck.routeSlotText || '') && /Boss-history signal/i.test(trophyPlanCheck.routeSlotText || '') && /No route access/i.test(trophyPlanCheck.routeSlotText || '') && /No reward access/i.test(trophyPlanCheck.routeSlotText || '') && /Anti-farming/i.test(trophyPlanCheck.routeSlotText || '') && trophyPlanCheck.routeSlotButtons.every(btn => /Route Locked/i.test(btn)) && trophyPlanCheck.routeSlotActionAttrs.length === 0, JSON.stringify({ routeSlotText: String(trophyPlanCheck.routeSlotText || '').slice(0, 900), routeSlotButtons: trophyPlanCheck.routeSlotButtons, routeSlotActionAttrs: trophyPlanCheck.routeSlotActionAttrs }));

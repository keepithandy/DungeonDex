...
async function main() {
  ...
  try {
    ...
    const mutationCheck = await evalByValue(client, `(() => {
      ...
      const laneUnsafeFields = [firstLane, secondLane].filter(Boolean).map(lane => ({
        laneKey: String(lane?.laneKey || ''),
        fields: ['entry', 'enter', 'start', 'travel', 'begin', 'action', 'activate', 'playable', 'reward', 'rewards', 'completion', 'complete', 'combat', 'scaling', 'scale'].filter(field => Object.prototype.hasOwnProperty.call(lane || {}, field))
      }));
    ...
    record('Third revisit lane is Rival Trace and remains planned only', mutationCheck.previewStateSummary && Array.isArray(mutationCheck.previewStateSummary.stableHookLabels) && mutationCheck.previewStateSummary.stableHookLabels.includes('rival_trace / board_echo') && mutationCheck.previewStateSummary.totalPreviewCount === 3 && mutationCheck.previewStateSummary.lockedPreviewCount === 3, JSON.stringify(mutationCheck.previewStateSummary));
    ...
    record('Third revisit route does not mutate player or run state', mutationCheck.snapshot === mutationCheck.after && mutationCheck.previewStateSummary && mutationCheck.previewStateSummary.currentPreviewOnly === true, JSON.stringify({ before: mutationCheck.snapshot, after: mutationCheck.after, summary: mutationCheck.previewStateSummary }));
    record('Lane contracts expose no route activation flags', Array.isArray(mutationCheck.laneUnsafeFields) && mutationCheck.laneUnsafeFields.length === 2 && mutationCheck.laneUnsafeFields.every(entry => entry.fields.length === 0) && [mutationCheck.firstLane, mutationCheck.secondLane].every(lane => lane && lane.hasLiveEntry === false && lane.hasRewards === false && lane.hasCompletion === false && lane.previewOnly === true), JSON.stringify({ unsafeFields: mutationCheck.laneUnsafeFields, firstLane: mutationCheck.firstLane, secondLane: mutationCheck.secondLane }));

...
async function main() {
  ...
  try {
    ...
    record('Route rows expose progress and safety copy', Array.isArray(mutationCheck.routeCards) && mutationCheck.routeCards.length >= 3 && mutationCheck.routeSlotButtons.length === 0, JSON.stringify({ routeSlotButtons: mutationCheck.routeSlotButtons, routeCards: mutationCheck.routeCards?.slice(0, 3) }));
    ...
    record('Trophy Echo rule chain remains handoff-safe', trophyPlanCheck.plan && trophyPlanCheck.summary && trophyPlanCheck.highEdge?.plan?.locked === true && trophyPlanCheck.highEdge.plan.ready === false && trophyPlanCheck.highEdge.plan.playable === false && trophyPlanCheck.highEdge.plan.active === false && trophyPlanCheck.highEdge.plan.rewardAvailable === false && trophyPlanCheck.malformedPlan?.locked === true && trophyPlanCheck.malformedPlan.ready === false && trophyPlanCheck.malformedPlan.playable === false && trophyPlanCheck.malformedPlan.active === false && trophyPlanCheck.malformedPlan.rewardAvailable === false && trophyPlanCheck.routeSlotButtons.length === 0 && trophyPlanCheck.routeSlotActionAttrs.length === 0 && trophyPlanCheck.apiKeys.every(key => !/revisit.*(enter|entry|start|travel|begin|claim|complete|reward|teleport|rerun|scale|activate|launch)/i.test(key)), JSON.stringify({ plan: !!trophyPlanCheck.plan, summary: !!trophyPlanCheck.summary, high: trophyPlanCheck.highEdge?.plan, malformed: trophyPlanCheck.malformedPlan, routeSlotButtons: trophyPlanCheck.routeSlotButtons, routeSlotActionAttrs: trophyPlanCheck.routeSlotActionAttrs }));

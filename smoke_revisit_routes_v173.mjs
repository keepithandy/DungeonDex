...
async function main(){
  ...
  try {
    ...
    record('Trophy Echo contract stays preview-only', audit.previewContractSafe && audit.trophyLaneSafe && audit.famousLaneSafe && audit.firstLane?.laneKey === 'trophy-echo' && audit.firstLane?.locked === true && audit.firstLane?.readOnly === true && audit.firstLane?.hasRewards === false && audit.firstLane?.hasCompletion === false && audit.secondLane?.laneKey === 'famous-gear-memory' && audit.secondLane?.locked === true && audit.secondLane?.readOnly === true, JSON.stringify({ trophyRoute:audit.trophyRoute, famousGearRoute:audit.famousGearRoute, firstLane:audit.firstLane, secondLane:audit.secondLane }));

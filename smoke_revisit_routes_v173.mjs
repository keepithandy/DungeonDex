...
async function main() {
  ...
  try {
    ...
    record('First revisit lane stays Trophy Echo even when preview order is reversed and a second lane exists', !!mutationCheck.reversedFirstLane && mutationCheck.reversedFirstLane.key === 'trophy_echo_route' && mutationCheck.reversedFirstLane.title === mutationCheck.firstLane?.routeLabel && !!mutationCheck.secondLane && mutationCheck.secondLane.laneKey === 'famous-gear-memory' && mutationCheck.firstLane?.laneKey === 'trophy-echo' && mutationCheck.firstLane?.sourceHook === 'Trophy Echo', JSON.stringify({ firstLane: mutationCheck.firstLane, secondLane: mutationCheck.secondLane, reversedFirstLane: mutationCheck.reversedFirstLane, reversedRoutes: mutationCheck.reversedRoutes?.map(route => route?.key) }));
    record('Trophy Echo remains the first lane with additional planned lane fixture data', Array.isArray(mutationCheck.laneFixtureWithSecondFirst) && mutationCheck.laneFixtureWithSecondFirst[0]?.laneKey === 'famous-gear-memory' && mutationCheck.fixtureTrophyLane?.laneKey === 'trophy-echo' && mutationCheck.fixtureSecondLane?.laneKey === 'famous-gear-memory' && mutationCheck.firstLane?.laneKey === 'trophy-echo' && mutationCheck.firstLane?.currentStatus === 'planned' && mutationCheck.secondLane?.currentStatus === 'planned', JSON.stringify({ firstLane: mutationCheck.firstLane, secondLane: mutationCheck.secondLane, fixture: mutationCheck.laneFixtureWithSecondFirst }));
    record('First revisit lane ignores candidate ordering while second-lane fixture data is present', Array.isArray(mutationCheck.reversedCandidateHooks) && mutationCheck.reversedCandidateHooks.some(hook => hook?.key === 'trophy_echo') && mutationCheck.reversedCandidateHooks.some(hook => hook?.key === 'famous_gear_memory') && mutationCheck.firstLane?.laneKey === 'trophy-echo' && mutationCheck.secondLane?.laneKey === 'famous-gear-memory', JSON.stringify({ reversedCandidateKeys: mutationCheck.reversedCandidateHooks?.map(hook => hook?.key), firstLane: mutationCheck.firstLane, secondLane: mutationCheck.secondLane }));

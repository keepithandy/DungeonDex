...
function collectLiveActionKeys(value, path = '', hits = []) {
  ...
  if (Array.isArray(value)) {
    value.forEach((entry, index) => collectLiveActionKeys(entry, `${path}[${index}]`, hits));
    return hits;
  }
  for (const [key, child] of Object.entries(value)) {
    const nextPath = path ? `${path}.${key}` : key;
    if (/^(enter|start|begin|claim|complete|reward|unlock|routeId|activeRoute|completionState|action|actions|button|cta|href|onClick|run|launch|activate)$/i.test(key)) hits.push(nextPath);
    collectLiveActionKeys(child, nextPath, hits);
  }
  return hits;
}
...
async function main() {
  ...
  try {
    ...
    const fresh = await evalByValue(client, `(() => {
      ...
      const contentDefs = typeof api.revisitRouteContentDefinitions === 'function' ? api.revisitRouteContentDefinitions() : null;
      ...
      const routeGates = typeof api.revisitRouteGateSummary === 'function' ? api.revisitRouteGateSummary(S) : null;
      ...
      const routeGateSamples = Array.isArray(routes) && typeof api.revisitRouteGateState === 'function' ? routes.map(route => api.revisitRouteGateState(S, route)) : [];
      const thirdRoute = Array.isArray(routes) ? routes.find(route => String(route?.key || '') === 'rival_trace_route') || null : null;
      const routeKeys = Array.isArray(routes) ? routes.map(route => String(route?.key || '')) : [];
      const trophyDetail = typeof api.revisitTrophyEchoRouteDetail === 'function' ? api.revisitTrophyEchoRouteDetail(S) : null;
      const trophyFallbacks = [
        { name: 'null state', value: typeof api.revisitTrophyEchoRouteDetail === 'function' ? api.revisitTrophyEchoRouteDetail(null) : null },
        { name: 'empty state', value: typeof api.revisitTrophyEchoRouteDetail === 'function' ? api.revisitTrophyEchoRouteDetail({}) : null },
        { name: 'null revisitState', value: typeof api.revisitTrophyEchoRouteDetail === 'function' ? api.revisitTrophyEchoRouteDetail({ revisitState: null }) : null },
        { name: 'partial revisitState', value: typeof api.revisitTrophyEchoRouteDetail === 'function' ? api.revisitTrophyEchoRouteDetail({ revisitState: { unlocked: true }, routePlan: null }) : null },
        {
          name: 'missing routePlan partial state',
          value: typeof api.revisitTrophyEchoRouteDetail === 'function'
            ? api.revisitTrophyEchoRouteDetail({ revisitState: { unlocked: true } })
            : null
        }
      ];
      ...
      const routeGatesAgain = typeof api.revisitRouteGateSummary === 'function' ? api.revisitRouteGateSummary(S) : null;
      ...
      const routeGateSamplesAgain = Array.isArray(routes) && typeof api.revisitRouteGateState === 'function' ? routes.map(route => api.revisitRouteGateState(S, route)) : [];
    ...
    record('Revisit activation lane order is exactly Trophy Echo, Famous Gear Memory, Rival Trace', Array.isArray(fresh.routes) && fresh.routes.length === 3 && fresh.routes[0]?.key === 'trophy_echo_route' && fresh.routes[1]?.key === 'famous_gear_route' && fresh.routes[2]?.key === 'rival_trace_route' && JSON.stringify(fresh.routeKeys) === JSON.stringify(['trophy_echo_route','famous_gear_route','rival_trace_route']), JSON.stringify({ routeKeys: fresh.routeKeys, routes: fresh.routes?.map(route => ({ key: route?.key, title: route?.title })) }));
    record('Trophy Echo detail contract is stable and read-only', fresh.trophyDetail && fresh.trophyDetail.key === 'trophy_echo_route' && fresh.trophyDetail.title === 'Trophy Echo Route' && fresh.trophyDetail.source === 'boss trophies / trophy records' && fresh.trophyDetail.status === 'Planning only' && fresh.trophyDetail.planningOnly === true && fresh.trophyDetail.locked === true && fresh.trophyDetail.readOnly === true && /planned memory detail lane/i.test(fresh.trophyDetail.detail || '') && /future side-route concept tied to remembered boss trophies/i.test(fresh.trophyDetail.detail || '') && /first planned revisit route/i.test(fresh.trophyDetail.reason || '') && /route access is unavailable/i.test(fresh.trophyDetail.safety || '') && typeof fresh.trophyDetail.signalCurrent === 'number' && typeof fresh.trophyDetail.signalRequired === 'number' && typeof fresh.trophyDetail.signalPercent === 'number' && !Object.prototype.hasOwnProperty.call(fresh.trophyDetail, 'entryButton') && !Object.prototype.hasOwnProperty.call(fresh.trophyDetail, 'startButton') && !Object.prototype.hasOwnProperty.call(fresh.trophyDetail, 'beginButton') && !Object.prototype.hasOwnProperty.call(fresh.trophyDetail, 'claimButton') && !Object.prototype.hasOwnProperty.call(fresh.trophyDetail, 'completeButton') && !Object.prototype.hasOwnProperty.call(fresh.trophyDetail, 'unlockButton') && !Object.prototype.hasOwnProperty.call(fresh.trophyDetail, 'rewardButton') && !Object.prototype.hasOwnProperty.call(fresh.trophyDetail, 'progressionMutation'), JSON.stringify(fresh.trophyDetail));
    record('Trophy Echo detail fallback cases remain locked and read-only', Array.isArray(fresh.trophyFallbacks) && fresh.trophyFallbacks.length === 5 && fresh.trophyFallbacks.every(entry => entry && typeof entry.value === 'object' && entry.value.key === 'trophy_echo_route' && entry.value.title === 'Trophy Echo Route' && entry.value.locked === true && entry.value.readOnly === true && entry.value.planningOnly === true && /route access is unavailable/i.test(String(entry.value.safety || '')) && !['entry','start','begin','claim','complete','unlock','reward','rewards','progression','progress','currency','payout','routeEntry','startRoute','completeRoute'].some(field => Object.prototype.hasOwnProperty.call(entry.value, field)) && !/^\s*(Enter|Start|Begin|Claim|Complete|Unlock|Reward|Payout)\b/i.test(String(entry.value.status || '')) && !/^\s*(Enter|Start|Begin|Claim|Complete|Unlock|Reward|Payout)\b/i.test(String(entry.value.reason || '')) && !/^\s*(Enter|Start|Begin|Claim|Complete|Unlock|Reward|Payout)\b/i.test(String(entry.value.detail || ''))), JSON.stringify(fresh.trophyFallbacks));
    const liveActionObjectHits = {
      routes: collectLiveActionKeys(fresh.routes),
      routeSummary: collectLiveActionKeys(fresh.routeSummary),
      activationPlan: collectLiveActionKeys(fresh.activationPlan),
      previewStateSummary: collectLiveActionKeys(fresh.previewStateSummary),
      firstLane: collectLiveActionKeys(fresh.firstLane),
      trophyDetail: collectLiveActionKeys(fresh.trophyDetail),
      trophyFallbacks: collectLiveActionKeys(fresh.trophyFallbacks),
      routeGates: collectLiveActionKeys(fresh.routeGates),
    ...
    record('Route gate helper exists and returns stable locked gate objects', fresh.routeGates && fresh.routeGates.total === fresh.routes.length && fresh.routeGates.locked === fresh.routes.length && fresh.routeGates.enterable === 0 && fresh.routeGates.previewOnly === true && Array.isArray(fresh.routeGateSamples) && fresh.routeGateSamples.length === fresh.routes.length && JSON.stringify(fresh.routeGateSamples) === JSON.stringify(fresh.routeGateSamplesAgain) && JSON.stringify(fresh.routeGates) === JSON.stringify(fresh.routeGatesAgain) && fresh.routeGateSamples.every(gate => gate && gate.locked === true && gate.enterable === false && gate.previewOnly === true && typeof gate.reason === 'string' && typeof gate.requirementLabel === 'string' && typeof gate.safetyLabel === 'string' && typeof gate.routeKey === 'string'), JSON.stringify({ routeGates: fresh.routeGates, routeGateSamples: fresh.routeGateSamples?.slice(0, 3) }));
    record('Revisit route content definitions keep the third lane planning-only and do not add a fourth activation lane', fresh.contentDefs && fresh.contentDefs.trophy_echo_route && fresh.contentDefs.famous_gear_route && fresh.contentDefs.rival_trace_route && !fresh.contentDefs.fourth_lane && fresh.routes.length === 3 && fresh.routeKeys.length === 3 && fresh.routeKeys[fresh.routeKeys.length - 1] === 'rival_trace_route' && fresh.contentDefs.rival_trace_route.title === 'Rival Trace Route' && /named rival elite memory leaves a trace/i.test(fresh.contentDefs.rival_trace_route.shortDescription || '') && /named rival elite memory/i.test(fresh.contentDefs.rival_trace_route.lockedReadinessNote || ''), JSON.stringify({ routeKeys: fresh.routeKeys, contentDefs: fresh.contentDefs }));
    ...
    record('Town revisit slot is removed from Town', !fresh.hasPlannedRoutes && !fresh.hasEntryGate && !fresh.hasRouteStillLocked && !fresh.hasRouteCards && !/Earlier Dungeon Revisit|Planned Return Routes/i.test(fresh.text || ''), JSON.stringify({ text: fresh.text.slice(0, 500), routeCards: fresh.routeCards, routeCardButtons: fresh.routeCardButtons }));
    ...
    const mutationCheck = await evalByValue(client, `(() => {
      ...
      const routes = typeof api.revisitRoutePreviews === 'function' ? api.revisitRoutePreviews(S) : [];
      const routeSummary = typeof api.revisitRouteSummary === 'function' ? api.revisitRouteSummary(S) : null;
      const activationPlan = typeof api.revisitRouteActivationPlan === 'function' ? api.revisitRouteActivationPlan(S) : null;
      const activationPlanAgain = typeof api.revisitRouteActivationPlan === 'function' ? api.revisitRouteActivationPlan(S) : null;
      const activationSummary = typeof api.revisitRouteActivationSummary === 'function' ? api.revisitRouteActivationSummary(S) : null;
      const activationSummaryAgain = typeof api.revisitRouteActivationSummary === 'function' ? api.revisitRouteActivationSummary(S) : null;
      const previewStateSummary = typeof api.revisitRoutePreviewStateSummary === 'function' ? api.revisitRoutePreviewStateSummary(S) : null;
      const previewStateSummaryAgain = typeof api.revisitRoutePreviewStateSummary === 'function' ? api.revisitRoutePreviewStateSummary(S) : null;
      const firstLane = typeof api.revisitFirstActivationLane === 'function' ? api.revisitFirstActivationLane(S) : null;
      const firstLaneAgain = typeof api.revisitFirstActivationLane === 'function' ? api.revisitFirstActivationLane(S) : null;
      const secondLane = typeof api.revisitSecondActivationLane === 'function' ? api.revisitSecondActivationLane(S) : null;
      const secondLaneAgain = typeof api.revisitSecondActivationLane === 'function' ? api.revisitSecondActivationLane(S) : null;
      const thirdLane = typeof api.revisitThirdActivationLane === 'function' ? api.revisitThirdActivationLane(S) : null;
      const thirdLaneAgain = typeof api.revisitThirdActivationLane === 'function' ? api.revisitThirdActivationLane(S) : null;
      const reversedRoutes = Array.isArray(routes) ? routes.slice().reverse() : [];
      const reversedFirstLane = reversedRoutes.find(route => String(route?.key || '') === 'trophy_echo_route') || null;
      const reversedSecondLane = reversedRoutes.find(route => String(route?.key || '') === 'famous_gear_route') || null;
      const reversedCandidateHooks = Array.isArray(hooks) ? hooks.slice().reverse() : [];
      const laneFixtureWithSecondFirst = [secondLane, firstLane].filter(Boolean);
      const fixtureTrophyLane = laneFixtureWithSecondFirst.find(lane => String(lane?.laneKey || '') === 'trophy-echo') || null;
      const fixtureSecondLane = laneFixtureWithSecondFirst.find(lane => String(lane?.laneKey || '') === 'famous-gear-memory') || null;
      const laneUnsafeFields = [firstLane, secondLane, thirdLane].filter(Boolean).map(lane => ({
        laneKey: String(lane?.laneKey || ''),
        fields: ['entry', 'enter', 'start', 'travel', 'begin', 'action', 'activate', 'playable', 'reward', 'rewards', 'completion', 'complete', 'unlock', 'unlocks', 'currency', 'progression', 'mutation', 'hunt', 'boardMission', 'mission', 'combat', 'scaling', 'scale'].filter(field => Object.prototype.hasOwnProperty.call(lane || {}, field))
      }));
    ...
    record('Activation rule contract exists and is stable', !!mutationCheck.activationPlan && mutationCheck.activationPlan.contractId === 'revisit_route_activation_contract_v1' && JSON.stringify(mutationCheck.activationPlan) === JSON.stringify(mutationCheck.activationPlanAgain), JSON.stringify(mutationCheck.activationPlan));
    record('Activation summary contract exists and is stable', !!mutationCheck.activationSummary && mutationCheck.activationSummary.contractId === 'revisit_route_activation_contract_v1' && JSON.stringify(mutationCheck.activationSummary) === JSON.stringify(mutationCheck.activationSummaryAgain), JSON.stringify(mutationCheck.activationSummary));
    record('Route-preview summary mirror exists and is stable', !!mutationCheck.previewStateSummary && mutationCheck.previewStateSummary.contractId === 'revisit_route_preview_state_summary_v1' && JSON.stringify(mutationCheck.previewStateSummary) === JSON.stringify(mutationCheck.previewStateSummaryAgain), JSON.stringify(mutationCheck.previewStateSummary));
    record('First revisit lane contract exists and is stable', !!mutationCheck.firstLane && mutationCheck.firstLane.laneKey === 'trophy-echo' && mutationCheck.firstLane.laneLabel === 'Trophy Echo' && mutationCheck.firstLane.currentStatus === 'planned' && JSON.stringify(mutationCheck.firstLane) === JSON.stringify(mutationCheck.firstLaneAgain), JSON.stringify(mutationCheck.firstLane));
    record('Second revisit lane contract exists and is stable', !!mutationCheck.secondLane && mutationCheck.secondLane.laneKey === 'famous-gear-memory' && mutationCheck.secondLane.laneLabel === 'Famous Gear Memory' && mutationCheck.secondLane.currentStatus === 'planned' && JSON.stringify(mutationCheck.secondLane) === JSON.stringify(mutationCheck.secondLaneAgain), JSON.stringify(mutationCheck.secondLane));
    record('Third revisit lane contract exists and is stable', !!mutationCheck.thirdLane && mutationCheck.thirdLane.laneKey === 'rival-trace' && mutationCheck.thirdLane.laneLabel === 'Rival Trace' && mutationCheck.thirdLane.currentStatus === 'planned' && JSON.stringify(mutationCheck.thirdLane) === JSON.stringify(mutationCheck.thirdLaneAgain), JSON.stringify(mutationCheck.thirdLane));
    record('First revisit lane stays Trophy Echo even when preview order is reversed and later routes exist', !!mutationCheck.reversedFirstLane && mutationCheck.reversedFirstLane.key === 'trophy_echo_route' && mutationCheck.reversedFirstLane.title === mutationCheck.firstLane?.routeLabel && !!mutationCheck.reversedRoutes && mutationCheck.reversedRoutes.some(route => route?.key === 'famous_gear_route') && mutationCheck.reversedRoutes.some(route => route?.key === 'rival_trace_route') && mutationCheck.firstLane?.laneKey === 'trophy-echo' && mutationCheck.firstLane?.sourceHook === 'Trophy Echo', JSON.stringify({ firstLane: mutationCheck.firstLane, secondLane: mutationCheck.secondLane, reversedFirstLane: mutationCheck.reversedFirstLane, reversedRoutes: mutationCheck.reversedRoutes?.map(route => route?.key) }));
    record('Trophy Echo remains first with additional planned route fixture data', Array.isArray(mutationCheck.laneFixtureWithSecondFirst) && mutationCheck.laneFixtureWithSecondFirst[0]?.laneKey === 'famous-gear-memory' && mutationCheck.fixtureTrophyLane?.laneKey === 'trophy-echo' && mutationCheck.fixtureSecondLane?.laneKey === 'famous-gear-memory' && mutationCheck.firstLane?.laneKey === 'trophy-echo' && mutationCheck.firstLane?.currentStatus === 'planned' && mutationCheck.secondLane?.currentStatus === 'planned', JSON.stringify({ firstLane: mutationCheck.firstLane, secondLane: mutationCheck.secondLane, fixture: mutationCheck.laneFixtureWithSecondFirst }));
    record('First revisit lane ignores candidate ordering while later route fixture data is present', Array.isArray(mutationCheck.reversedCandidateHooks) && mutationCheck.reversedCandidateHooks.some(hook => hook?.key === 'trophy_echo') && mutationCheck.reversedCandidateHooks.some(hook => hook?.key === 'famous_gear_memory') && mutationCheck.reversedCandidateHooks.some(hook => hook?.key === 'rival_trace') && mutationCheck.firstLane?.laneKey === 'trophy-echo' && mutationCheck.secondLane?.laneKey === 'famous-gear-memory', JSON.stringify({ reversedCandidateKeys: mutationCheck.reversedCandidateHooks?.map(hook => hook?.key), firstLane: mutationCheck.firstLane, secondLane: mutationCheck.secondLane }));
    record('Activation rule contract stays planning only and inert', mutationCheck.activationPlan && mutationCheck.activationPlan.status === 'Planning only' && mutationCheck.activationPlan.locked === true && mutationCheck.activationPlan.readOnly === true && mutationCheck.activationPlan.entryAvailable === false && mutationCheck.activationPlan.rewardAvailable === false && mutationCheck.activationPlan.completionAvailable === false && mutationCheck.activationPlan.optionalSideContent === true && mutationCheck.activationPlan.primaryPath === 'Enter Dungeon / Continue Run' && Array.isArray(mutationCheck.activationPlan.allowedFutureRouteStates) && JSON.stringify(mutationCheck.activationPlan.allowedFutureRouteStates) === JSON.stringify(['locked', 'planned', 'eligible-preview', 'playable-later']) && Array.isArray(mutationCheck.activationPlan.stableHookSources) && mutationCheck.activationPlan.stableHookSources.length === 5 && Array.isArray(mutationCheck.activationPlan.routeStates) && mutationCheck.activationPlan.routeStates.length === 3 && mutationCheck.activationPlan.eligibleRoutes.length === 3 && mutationCheck.activationPlan.eligibleRoutes.every(route => route.locked === true && route.readOnly === true && route.entryAvailable === false && route.rewardAvailable === false && route.completionAvailable === false && ['eligible-preview', 'playable-later'].includes(route.eligibilityState) && route.optionalSideContent === true && route.primaryPathPreserved === true && route.sourceHistoryOnly === true && ['trophy_echo_route','famous_gear_route','rival_trace_route'].includes(route.key)) && !Object.prototype.hasOwnProperty.call(mutationCheck.activationPlan, 'entryButton') && !Object.prototype.hasOwnProperty.call(mutationCheck.activationPlan, 'routeButton'), JSON.stringify(mutationCheck.activationPlan));
    record('Activation summary stays preview-only and inert', mutationCheck.activationSummary && Array.isArray(mutationCheck.activationSummary.allowedStates) && JSON.stringify(mutationCheck.activationSummary.allowedStates) === JSON.stringify(['locked', 'planned', 'eligible-preview', 'playable-later']) && mutationCheck.activationSummary.currentPreviewOnly === true && mutationCheck.activationSummary.hasLiveEntry === false && mutationCheck.activationSummary.hasRewards === false && mutationCheck.activationSummary.hasCompletion === false && mutationCheck.activationSummary.currentPlayableCount === 0 && mutationCheck.activationSummary.currentLockedCount === 3 && typeof mutationCheck.activationSummary.note === 'string' && /preview-state vocabulary only/i.test(mutationCheck.activationSummary.note), JSON.stringify(mutationCheck.activationSummary));
    record('Route-preview summary mirror stays preview-only and inert', mutationCheck.previewStateSummary && Array.isArray(mutationCheck.previewStateSummary.allowedStates) && JSON.stringify(mutationCheck.previewStateSummary.allowedStates) === JSON.stringify(['locked', 'planned', 'eligible-preview', 'playable-later']) && mutationCheck.previewStateSummary.currentPreviewOnly === true && mutationCheck.previewStateSummary.playablePreviewCount === 0 && mutationCheck.previewStateSummary.hasLiveEntry === false && mutationCheck.previewStateSummary.hasRewards === false && mutationCheck.previewStateSummary.hasCompletion === false && mutationCheck.previewStateSummary.totalPreviewCount === 3 && mutationCheck.previewStateSummary.lockedPreviewCount === 3 && Array.isArray(mutationCheck.previewStateSummary.stableHookLabels) && mutationCheck.previewStateSummary.stableHookLabels.length >= 3 && typeof mutationCheck.previewStateSummary.note === 'string' && /route-preview mirror only/i.test(mutationCheck.previewStateSummary.note) && mutationCheck.previewStateSummary.stableHookLabels.includes('rival_trace \/ board_echo'), JSON.stringify(mutationCheck.previewStateSummary));
    record('First revisit lane is Trophy Echo and remains planned only', mutationCheck.firstLane && mutationCheck.firstLane.laneKey === 'trophy-echo' && mutationCheck.firstLane.laneLabel === 'Trophy Echo' && mutationCheck.firstLane.sourceHook === 'Trophy Echo' && mutationCheck.firstLane.currentStatus === 'planned' && mutationCheck.firstLane.locked === true && mutationCheck.firstLane.readOnly === true && mutationCheck.firstLane.previewOnly === true && mutationCheck.firstLane.hasLiveEntry === false && mutationCheck.firstLane.hasRewards === false && mutationCheck.firstLane.hasCompletion === false && Array.isArray(mutationCheck.firstLane.allowedStates) && JSON.stringify(mutationCheck.firstLane.allowedStates) === JSON.stringify(['locked', 'planned', 'eligible-preview', 'playable-later']) && /existing boss and trophy history only/i.test(mutationCheck.firstLane.reason || '') && /first planned revisit lane/i.test(mutationCheck.firstLane.note || '') && !Object.prototype.hasOwnProperty.call(mutationCheck.firstLane, 'entry') && !Object.prototype.hasOwnProperty.call(mutationCheck.firstLane, 'reward') && !Object.prototype.hasOwnProperty.call(mutationCheck.firstLane, 'completion'), JSON.stringify(mutationCheck.firstLane));
    record('Second revisit lane is Famous Gear Memory and remains planned only', mutationCheck.secondLane && mutationCheck.secondLane.laneKey === 'famous-gear-memory' && mutationCheck.secondLane.laneLabel === 'Famous Gear Memory' && mutationCheck.secondLane.sourceHook === 'Famous Gear Memory' && mutationCheck.secondLane.currentStatus === 'planned' && mutationCheck.secondLane.locked === true && mutationCheck.secondLane.readOnly === true && mutationCheck.secondLane.previewOnly === true && mutationCheck.secondLane.hasLiveEntry === false && mutationCheck.secondLane.hasRewards === false && mutationCheck.secondLane.hasCompletion === false && mutationCheck.secondLane.routeKey === 'famous_gear_route' && mutationCheck.reversedSecondLane?.key === 'famous_gear_route' && Array.isArray(mutationCheck.secondLane.allowedStates) && JSON.stringify(mutationCheck.secondLane.allowedStates) === JSON.stringify(['locked', 'planned', 'eligible-preview', 'playable-later']) && /existing famous gear and archive memory only/i.test(mutationCheck.secondLane.reason || '') && /second planned revisit lane/i.test(mutationCheck.secondLane.note || '') && !Object.prototype.hasOwnProperty.call(mutationCheck.secondLane, 'entry') && !Object.prototype.hasOwnProperty.call(mutationCheck.secondLane, 'reward') && !Object.prototype.hasOwnProperty.call(mutationCheck.secondLane, 'completion'), JSON.stringify({ secondLane: mutationCheck.secondLane, reversedSecondLane: mutationCheck.reversedSecondLane }));
    record('Third revisit lane is Rival Trace and remains planning only', mutationCheck.thirdLane && mutationCheck.thirdLane.laneKey === 'rival-trace' && mutationCheck.thirdLane.laneLabel === 'Rival Trace' && mutationCheck.thirdLane.sourceHook === 'Rival Trace' && mutationCheck.thirdLane.currentStatus === 'planned' && mutationCheck.thirdLane.locked === true && mutationCheck.thirdLane.readOnly === true && mutationCheck.thirdLane.previewOnly === true && mutationCheck.thirdLane.hasLiveEntry === false && mutationCheck.thirdLane.hasRewards === false && mutationCheck.thirdLane.hasCompletion === false && mutationCheck.thirdLane.routeKey === 'rival_trace_route' && Array.isArray(mutationCheck.thirdLane.allowedStates) && JSON.stringify(mutationCheck.thirdLane.allowedStates) === JSON.stringify(['locked', 'planned', 'eligible-preview', 'playable-later']) && /remembered named rival elite history only/i.test(mutationCheck.thirdLane.reason || '') && /memory\/planning only/i.test(mutationCheck.thirdLane.note || '') && /no hunt, board mission, route, reward, unlock, currency, progression, or combat mutation/i.test(mutationCheck.thirdLane.note || '') && mutationCheck.previewStateSummary && Array.isArray(mutationCheck.previewStateSummary.stableHookLabels) && mutationCheck.previewStateSummary.stableHookLabels.includes('rival_trace / board_echo') && mutationCheck.previewStateSummary.totalPreviewCount === 3 && mutationCheck.previewStateSummary.lockedPreviewCount === 3, JSON.stringify({ thirdLane: mutationCheck.thirdLane, previewStateSummary: mutationCheck.previewStateSummary }));
    record('First revisit lane does not mutate player or run state', mutationCheck.snapshot === mutationCheck.after && mutationCheck.firstLane && mutationCheck.firstLane.previewOnly === true && mutationCheck.firstLane.locked === true, JSON.stringify({ before: mutationCheck.snapshot, after: mutationCheck.after, firstLane: mutationCheck.firstLane }));
    record('Second revisit lane does not mutate player or run state', mutationCheck.snapshot === mutationCheck.after && mutationCheck.secondLane && mutationCheck.secondLane.previewOnly === true && mutationCheck.secondLane.locked === true, JSON.stringify({ before: mutationCheck.snapshot, after: mutationCheck.after, secondLane: mutationCheck.secondLane }));
    record('Third revisit route does not mutate player or run state', mutationCheck.snapshot === mutationCheck.after && mutationCheck.thirdLane && mutationCheck.thirdLane.previewOnly === true && mutationCheck.thirdLane.locked === true && mutationCheck.previewStateSummary && mutationCheck.previewStateSummary.currentPreviewOnly === true, JSON.stringify({ before: mutationCheck.snapshot, after: mutationCheck.after, thirdLane: mutationCheck.thirdLane, summary: mutationCheck.previewStateSummary }));
    record('Lane contracts expose no route activation flags', Array.isArray(mutationCheck.laneUnsafeFields) && mutationCheck.laneUnsafeFields.length === 3 && mutationCheck.laneUnsafeFields.every(entry => entry.fields.length === 0) && [mutationCheck.firstLane, mutationCheck.secondLane, mutationCheck.thirdLane].every(lane => lane && lane.hasLiveEntry === false && lane.hasRewards === false && lane.hasCompletion === false && lane.previewOnly === true), JSON.stringify({ unsafeFields: mutationCheck.laneUnsafeFields, firstLane: mutationCheck.firstLane, secondLane: mutationCheck.secondLane, thirdLane: mutationCheck.thirdLane }));
    record('Route-preview summary mirror does not mutate player or run state', mutationCheck.snapshot === mutationCheck.after && mutationCheck.previewStateSummary && mutationCheck.previewStateSummary.currentPreviewOnly === true, JSON.stringify({ summary: mutationCheck.previewStateSummary, before: mutationCheck.snapshot, after: mutationCheck.after }));
    record('Activation summary does not imply gameplay state', mutationCheck.activationSummary && mutationCheck.activationSummary.hasLiveEntry === false && mutationCheck.activationSummary.hasRewards === false && mutationCheck.activationSummary.hasCompletion === false && mutationCheck.activationSummary.currentPreviewOnly === true && mutationCheck.snapshot === mutationCheck.after, JSON.stringify({ summary: mutationCheck.activationSummary, before: mutationCheck.snapshot, after: mutationCheck.after }));
    record('Activation plan state vocabulary is contract-safe', Array.isArray(mutationCheck.activationPlan?.routeStates) && mutationCheck.activationPlan.routeStates.every(entry => entry && ['locked', 'planned', 'eligible-preview', 'playable-later'].includes(entry.state) && !Object.prototype.hasOwnProperty.call(entry, 'reward') && !Object.prototype.hasOwnProperty.call(entry, 'completion') && !Object.prototype.hasOwnProperty.call(entry, 'entry') && !Object.prototype.hasOwnProperty.call(entry, 'playable') && !Object.prototype.hasOwnProperty.call(entry, 'active')), JSON.stringify(mutationCheck.activationPlan?.routeStates));
    record('Activation plan cannot imply live entry or rewards', mutationCheck.activationPlan && mutationCheck.activationPlan.entryAvailable === false && mutationCheck.activationPlan.rewardAvailable === false && mutationCheck.activationPlan.completionAvailable === false && !Object.prototype.hasOwnProperty.call(mutationCheck.activationPlan, 'playable') && !Object.prototype.hasOwnProperty.call(mutationCheck.activationPlan, 'entry') && !Object.prototype.hasOwnProperty.call(mutationCheck.activationPlan, 'reward') && !Object.prototype.hasOwnProperty.call(mutationCheck.activationPlan, 'completion'), JSON.stringify(mutationCheck.activationPlan));
    record('Activation rule contract does not mutate player or run state', mutationCheck.snapshot === mutationCheck.after && mutationCheck.activationPlan && mutationCheck.activationPlan.primaryPath === 'Enter Dungeon / Continue Run', JSON.stringify({ before: mutationCheck.snapshot, after: mutationCheck.after, activationPlan: mutationCheck.activationPlan }));
    record('Route previews expose no reward or completion keys', Array.isArray(mutationCheck.routePreviewFields) && mutationCheck.routePreviewFields.length === mutationCheck.previews.length && mutationCheck.routePreviewFields.every(entry => entry.rewardKeys.length === 0 && entry.completionKeys.length === 0 && entry.entryKeys.length === 0) && mutationCheck.previews.every(preview => preview.locked === true && preview.playable === false && preview.previewSafety && /route access is unavailable/i.test(preview.previewSafety)), JSON.stringify(mutationCheck.routePreviewFields));
    ...
    record('No revisit route entry API or button exists', mutationCheck.apiKeys.every(key => !/revisit.*(enter|entry|start|travel|begin|claim|complete|reward|teleport|rerun|scale|activate|launch)/i.test(key)) && mutationCheck.buttons.every(label => !/revisit|return route|route preview|enter route/i.test(label)) && mutationCheck.buttons.every(label => !/start revisit|enter revisit|continue route/i.test(label)), JSON.stringify({ apiKeys: mutationCheck.apiKeys, buttons: mutationCheck.buttons }));
    record('Route rows expose progress and safety copy', Array.isArray(mutationCheck.routeCards) && mutationCheck.routeCards.length === 0 && Array.isArray(mutationCheck.routeSlotButtons) && mutationCheck.routeSlotButtons.length === 0 && !/claim reward|route reward|complete route|finish route/i.test(mutationCheck.routeSlotText || ''), JSON.stringify({ routeSlotButtons: mutationCheck.routeSlotButtons, routeCards: mutationCheck.routeCards?.slice(0, 3) }));
    record('Town revisit UI omits route entry affordances', !/Start Revisit|Enter Revisit|Begin Route|Start Route|Continue Route/i.test(mutationCheck.routeSlotText || ''), JSON.stringify({ routeSlotText: String(mutationCheck.routeSlotText || '').slice(0, 500) }));
    record('Town revisit UI omits route reward and completion affordances', !/Claim Reward|Route Reward|Complete Route|Finish Route|Collect Reward/i.test(mutationCheck.routeSlotText || ''), JSON.stringify({ routeSlotText: String(mutationCheck.routeSlotText || '').slice(0, 500) }));
    record('Route cards expose no Enter Start Travel Begin or Continue Route action', Array.isArray(mutationCheck.routeSlotButtons) && mutationCheck.routeSlotButtons.every(btn => /Route Locked/i.test(btn)) && !/(Enter|Start|Travel|Begin|Continue)\\s+(Revisit|Route|Return)/i.test(mutationCheck.routeSlotText || ''), JSON.stringify({ routeSlotButtons: mutationCheck.routeSlotButtons, routeSlotText: String(mutationCheck.routeSlotText || '').slice(0, 500) }));

...
async function main() {
  ...
  try {
    ...
    record('Unlock gate helper exists and returns locked gate shapes', Array.isArray(fresh.gates) && fresh.gateSummary && fresh.gateSummary.total === fresh.gates.length && fresh.gateSummary.ready === 0 && fresh.gateSummary.playable === 0 && typeof fresh.gateSummary.progressAverage === 'number' && typeof fresh.gateSummary.progressNoted === 'number' && fresh.gates.every(gate => gate.locked === true && gate.ready === false && gate.playable === false && typeof gate.key === 'string' && typeof gate.label === 'string' && typeof gate.gateType === 'string' && typeof gate.reason === 'string' && typeof gate.requirement === 'string' && typeof gate.progressLabel === 'string' && typeof gate.progressCurrent === 'number' && typeof gate.progressRequired === 'number' && typeof gate.progressPercent === 'number' && Array.isArray(gate.signals) && gate.progressRequired >= 1 && gate.progressCurrent >= 0 && gate.progressPercent >= 0 && gate.progressPercent <= 100), JSON.stringify({ gates: fresh.gates?.slice(0, 3), gateSummary: fresh.gateSummary }));
    ...
    const malformed = await evalByValue(client, `(() => {
      ...
      const previewSummary = typeof api.revisitUnlockPreviewSummary === 'function' ? api.revisitUnlockPreviewSummary(S) : null;
      const text = document.getElementById('revisitFoundationSlot')?.innerText || '';
      return { before, afterNormalize, repaired, hooks, routes, gates, previews, summary, routeSummary, gateSummary, previewSummary, text };
    })()`);
    ...
    const mutationCheck = await evalByValue(client, `(() => {
      ...
      return { snapshot, after, hooks, summary, routes, routeSummary, gates, gateSummary, previews, previewSummary, candidateObjectsBefore, candidateObjectsAfter, routeObjectsBefore, routeObjectsAfter, previewObjectsBefore, previewObjectsAfter, apiKeys, buttons, routeSlotButtons, routeSlotText };
    })()`);
    ...
    record('Unlock gate summaries cannot imply playable route access', mutationCheck.gateSummary && mutationCheck.gateSummary.total === mutationCheck.gateSummary.locked && mutationCheck.gateSummary.ready === 0 && mutationCheck.gateSummary.playable === 0 && typeof mutationCheck.gateSummary.progressAverage === 'number' && typeof mutationCheck.gateSummary.progressNoted === 'number' && !mutationCheck.gateSummary.unlocked && !mutationCheck.gateSummary.available && !mutationCheck.gateSummary.entry && !mutationCheck.gateSummary.reward && !mutationCheck.gateSummary.completion && !mutationCheck.gateSummary.scaling, JSON.stringify(mutationCheck.gateSummary));
    ...
    record('Route rows expose progress and safety copy', /Earlier Dungeon Revisit/.test(mutationCheck.routeSlotText || '') && /Locked Preview/.test(mutationCheck.routeSlotText || '') && /Preview notes only/.test(mutationCheck.routeSlotText || '') && /Route access is unavailable/.test(mutationCheck.routeSlotText || '') && /Future Conditions/.test(mutationCheck.routeSlotText || '') && /Requirement:/.test(mutationCheck.routeSlotText || '') && /Safety:/.test(mutationCheck.routeSlotText || '') && /Progress:|Gate progress:/.test(mutationCheck.routeSlotText || '') && /Signal:/.test(mutationCheck.routeSlotText || '') && mutationCheck.routeSlotButtons.length === 0, JSON.stringify({ routeSlotText: String(mutationCheck.routeSlotText || '').slice(0, 500), routeSlotButtons: mutationCheck.routeSlotButtons }));

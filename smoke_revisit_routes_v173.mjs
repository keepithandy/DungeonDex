...
async function main() {
  ...
  try {
    ...
    record('Revisit route content definitions exist for all known routes', fresh.contentDefs && ['trophy_echo_route','famous_gear_route','rival_trace_route','debt_pressure_route','board_echo_route'].every(key => fresh.contentDefs[key] && typeof fresh.contentDefs[key].title === 'string' && typeof fresh.contentDefs[key].district === 'string' && typeof fresh.contentDefs[key].hookSource === 'string' && typeof fresh.contentDefs[key].shortDescription === 'string' && typeof fresh.contentDefs[key].routeFlavorLine === 'string' && typeof fresh.contentDefs[key].safetyStatusLine === 'string' && typeof fresh.contentDefs[key].lockedReadinessNote === 'string' && fresh.contentDefs[key].title.length > 0 && fresh.contentDefs[key].district.length > 0 && fresh.contentDefs[key].shortDescription.length > 0 && fresh.contentDefs[key].routeFlavorLine.length > 0 && fresh.contentDefs[key].safetyStatusLine.length > 0 && fresh.contentDefs[key].lockedReadinessNote.length > 0), JSON.stringify(fresh.contentDefs));

...
async function main() {
  ...
  try {
    ...
    record('First revisit lane stays Trophy Echo even when preview order is reversed', !!mutationCheck.reversedFirstLane && mutationCheck.reversedFirstLane.key === 'trophy_echo_route' && mutationCheck.reversedFirstLane.title === mutationCheck.firstLane?.routeLabel && mutationCheck.firstLane?.laneKey === 'trophy-echo' && mutationCheck.firstLane?.sourceHook === 'Trophy Echo', JSON.stringify({ firstLane: mutationCheck.firstLane, reversedFirstLane: mutationCheck.reversedFirstLane, reversedRoutes: mutationCheck.reversedRoutes?.map(route => route?.key) }));
    ...
    record('Town revisit UI omits route entry affordances', !/Start Revisit|Enter Revisit|Begin Route|Start Route/i.test(mutationCheck.routeSlotText || ''), JSON.stringify({ routeSlotText: String(mutationCheck.routeSlotText || '').slice(0, 500) }));

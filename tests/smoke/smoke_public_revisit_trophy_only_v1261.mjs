...
assert.match(lockdownSource, /1\.33\.0-guildbound/, 'lockdown report should use the v1.33.0 public build key');
assert.match(lockdownSource, /const VISIBLE_BUILD = '1\.33\.0';/, 'lockdown report should expose the v1.33.0 visible build');
...
console.log('PASS: v1.33.0 public Revisit is Trophy Echo-only across DOM markup, route previews, global mutation functions, and exported API methods.');

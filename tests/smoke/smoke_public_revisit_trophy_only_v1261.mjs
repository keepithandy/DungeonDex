...
assert.match(lockdownSource, /1\.31\.1-quiet-route-restore/, 'lockdown report should use the v1.31.1 public build key');
assert.match(lockdownSource, /const VISIBLE_BUILD = '1\.31\.1';/, 'lockdown report should expose the v1.31.1 visible build');
...
console.log('PASS: v1.31.1 public Revisit is Trophy Echo-only across DOM markup, route previews, global mutation functions, and exported API methods.');

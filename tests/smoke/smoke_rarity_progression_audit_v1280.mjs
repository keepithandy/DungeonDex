...
async function loadRuntime() {
  ...
  vm.runInContext(`globalThis.__rarityAuditApi = {
    ...
    BOSS_INTERVAL,
    DEPTH_CHAPTERS_PER_THREAT_STEP,
    mythicSetDropChance,
...
function secondBossDropChance(runtime, rawDepth, state) {
  // Measure the live monotone RNG threshold instead of copying its formula.
  let low = 0;
  let high = 1;
  for (let step = 0; step < 48; step += 1) {
    const roll = (low + high) / 2;
    runtime.setRandom(() => roll);
    if (runtime.api.shouldDropLoot(rawDepth, 'boss', 1, state)) low = roll;
    else high = roll;
  }
  return (low + high) / 2;
}
  ...
  for (const rawDepth of CHECKPOINTS) {
    ...
    for (const source of SOURCES) {
      ...
      rows.push({
        ...
        source,
        conditionalBossSource: source === 'boss' && rawDepth % (runtime.api.BOSS_INTERVAL * runtime.api.DEPTH_CHAPTERS_PER_THREAT_STEP) !== 0,
...
function simulateFirstCompleteSet(setCount, slotCount) {
  // Conditional independent uniform draws, not an encounter/playtime simulation.
  const drops = [];
  const duplicates = [];
  const uniquePieces = [];
  for (let trial = 0; trial < SET_COLLECTION_TRIALS; trial += 1) {
    const random = mulberry32(seedFor('rarity-audit-set', trial));
    const owned = Array.from({ length: setCount }, () => new Set());
    const seen = new Set();
    let dropCount = 0;
    while (!owned.some(slots => slots.size === slotCount)) {
      const pieceIndex = Math.floor(random() * setCount * slotCount);
      const setIndex = Math.floor(pieceIndex / slotCount);
      const slotIndex = pieceIndex % slotCount;
...
function printReport(rows, collection) {
  ...
  for (const row of rows) {
    console.log([
      `D${row.rawDepth}`,
      row.threat,
      row.source + (row.conditionalBossSource ? ' (hypothetical)' : ''),
...
for (let setIndex = 0; setIndex < setIds.length; setIndex += 1) {
  for (let slotIndex = 0; slotIndex < setSlots.length; slotIndex += 1) {
    const rolls = [(setIndex + 0.5) / setIds.length, (slotIndex + 0.5) / setSlots.length];
    runtime.setRandom(() => rolls.length ? rolls.shift() : 0.5);
    const piece = runtime.api.generateMythicSetPiece(40, 'normal', eligibleState);
    assert.equal(piece.setId, setIds[setIndex], 'runtime set selection must match the uniform model');
    assert.equal(piece.setSlot, setSlots[slotIndex], 'runtime slot selection must match the uniform model');
  }
}
const collection = simulateFirstCompleteSet(setIds.length, setSlots.length);

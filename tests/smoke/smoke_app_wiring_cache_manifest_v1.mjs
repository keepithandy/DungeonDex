...
const VISIBLE_BUILD = '1.31.0';
const RELEASE_NAME = 'Town, Gear & Guild Navigation Quality of Life';
const RELEASE_LABEL = `v${VISIBLE_BUILD} ${RELEASE_NAME}`;
const BUILD_QS = '1.31.0-town-gear-navigation-qol';
const PUBLIC_DEPLOYMENT_STATUS = 'The observed hosted runtime is labeled v1.28.2 and its latest downloadable package is DungeonDex_v1.27_ItchReady (2).zip. The v1.31.0 candidate is local only until an explicitly authorized upload.';
...
async function main() {
  ...
  if (failures.length) {
    console.error(`FAIL: ${failures.length} v${VISIBLE_BUILD} build/cache authority mismatch(es):`);

...
async function main() {
  ...
  const labelContracts = [
    ['VERSION.md current version', version, `v${VISIBLE_BUILD} Boss 2 Readiness + Scaling Audit`],
  ...
  if (labelMismatches.length) {
    console.error('Mixed or stale v1.26.3 build labels detected:');
  ...
  console.log(`PASS v1.26.3 build/cache labels align, public runtime excludes direct devtools loads, and sw.js caches only public assets (${uniqueDirectScripts.length + uniqueDirectStyles.length + uniqueLoads.length} checked)`);
}

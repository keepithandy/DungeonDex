...
async function main() {
  ...
  record('Public shell names the current game and Journal', index.includes('<title>DungeonDex v1.26.5</title>') && index.includes('aria-label="Guild Journal"'), 'title and Guild Journal surface');
  ...
  record('Fresh intro identifies the first descent without changing the live entry actions', [
    ...
    "isFirstDescent ? 'DungeonDex' : 'Hollow Stair Threshold'",
    "isFirstDescent ? 'Records from the Hollow Stair' : stagingDistrict.line",
    'Welcome to the Guild. Enter the Hollow Stair, survive what you can, and let the Journal remember what mattered.',
    "id=\"${actionId}\">Enter Dungeon",
  ...
  console.log(`\nPublic-copy v1.26.5 smoke: ${passed}/${results.length} passed`);

...
async function main() {
  ...
  try {
    ...
    record('Town sections retain readable v1.32 labels', /Lowfire Board/.test(before.board) && /Lowfire Market/.test(before.market) && /Lowfire Forge/.test(before.forge), JSON.stringify({ board: /Lowfire Board/.test(before.board), market: /Lowfire Market/.test(before.market), forge: /Lowfire Forge/.test(before.forge) }));
    record('Trophy Echo-only Revisit panel renders before dungeon entry', /Revisit/.test(before.board) && /Trophy Echo is the only active Revisit lane for v1\.32\.2/.test(before.board) && !/Start Famous Gear Memory|Start Rival Trace|Start Board Echo|Start Debt Pressure/.test(before.board), JSON.stringify({ revisitStartButtons: before.revisitStartButtons, board: before.board.slice(0, 1200) }));

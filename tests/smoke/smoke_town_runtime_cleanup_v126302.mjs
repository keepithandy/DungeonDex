...
for (const source of [index, app, serviceWorker]) {
  assert.ok(source.includes(build), 'public runtime surfaces should use the current v1.31.1 build query');
}
...
console.log('PASS: v1.26.3.02 Town runtime cleanup remains canonical under v1.31.1 and obsolete layers are retired.');

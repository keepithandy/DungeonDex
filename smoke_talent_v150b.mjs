...
async function main() {
  ...
  const getBossTrophyState = async () => await evalByValue(client, `(() => ({
    ...
    record('Learned debt_collector_clarity is preview-ready but remains inactive', earningAudit?.debtPassiveContractLearned?.nodeKey === TALENT_IDS.debtClarity && earningAudit?.debtPassiveContractLearned?.contractOwner === 'DungeonDexTalents' && earningAudit?.debtPassiveContractLearned?.learned === true && earningAudit?.debtPassiveContractLearned?.passiveReady === true && earningAudit?.debtPassiveContractLearned?.passiveEnabled === false && earningAudit?.debtPassiveContractLearned?.appliesEffect === false && earningAudit?.debtPassiveContractLearned?.liveRendererWired === false && earningAudit?.debtPassiveContractReplay?.passiveEnabled === false && JSON.stringify(earningAudit?.debtPassiveContractLearned) === JSON.stringify(earningAudit?.debtPassiveContractReplay), JSON.stringify(earningAudit?.debtPassiveContractLearned));

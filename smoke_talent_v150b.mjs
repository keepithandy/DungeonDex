...
async function main() {
  ...
  const getBossTrophyState = async () => await evalByValue(client, `(() => ({
    ...
    record('v1.21.0 duplicate spend blocks as already learned', v12049Duplicate?.ok === false && v12049Duplicate?.blockedReason === 'already_learned' && v12049AfterSpend?.save?.availablePoints === 0 && v12049AfterSpend?.save?.spentPoints === 1 && v12049AfterSpend?.learned === true, JSON.stringify(v12049Duplicate));
    await client.send('Page.reload', { ignoreCache: true });
    await waitForCondition(client, `!!window.DungeonDexScenarioDevTools && !!window.DungeonDexTalents && typeof render === 'function' && typeof S !== 'undefined' && !!S && !!S.player && document.body && document.readyState !== 'loading'`, 15000);

...
async function main() {
  ...
  const getBossTrophyState = async () => await evalByValue(client, `(() => ({
    ...
    record('Board copy helper formats passive text without mutating input', typeof unlearnedBoardText === 'string' && unlearnedBoardText.includes('Target:') && unlearnedBoardText.includes('Objective:') && unlearnedBoardText.includes('Reward preview:'), unlearnedBoardText.slice(0, 700));
    ...
    record('Controlled talent action text stays limited to the approved spend path', Array.isArray(talentActionTextAudit?.matches) && talentActionTextAudit.matches.every(text => /spend 1 talent point/i.test(text)) && talentActionTextAudit.matches.length === 1, Array.isArray(talentActionTextAudit?.texts) ? talentActionTextAudit.texts.slice(0, 20).join(' | ') : '');
    ...
    record('Learned spend mutates only the intended learned state', !!learnedSpendResult && learnedSpendResult.ok === true && learnedSpendFixture?.player?.talentLearnedIds?.hunter_board_clarity === true && Array.isArray(learnedSpendFixture?.player?.talentUnlockIds) && learnedSpendFixture.player.talentUnlockIds.includes('hunter_board_clarity') && learnedSpendFixture?.player?.talentLedger?.spentPoints === 1 && learnedSpendFixture?.player?.talentLedger?.availablePoints === 0, JSON.stringify({ before: learnedSaveBefore, after: JSON.stringify(learnedSpendFixture) }));
    ...
    record('v1.23.1 build label is visible after reload', v12049Api?.build === '1.23.1' && String(v12049Api?.visibleLabel || '').includes('v1.23.1'), JSON.stringify({ build: v12049Api?.build, visibleLabel: v12049Api?.visibleLabel }));

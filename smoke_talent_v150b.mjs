...
async function main() {
  ...
  const getBossTrophyState = async () => await evalByValue(client, `(() => ({
    ...
    learnedSaveFixture.player.talentEarning.pointsAwarded = 1;
    ...
    record('No new talent action text appears in the UI', Array.isArray(talentActionTextAudit?.matches) && talentActionTextAudit.matches.length === 0 && Array.isArray(talentActionTextAudit?.texts) && talentActionTextAudit.texts.filter(text => text === 'Spend 1 Talent Point').length <= 1, Array.isArray(talentActionTextAudit?.texts) ? talentActionTextAudit.texts.slice(0, 20).join(' | ') : '');
    ...
    record('Learned spend updates the canonical spend fixture', learnedSpendResult?.ok === true && learnedSpendResult?.nodeKey === 'hunter_board_clarity' && learnedSpendResult?.availableBefore === 1 && learnedSpendResult?.availableAfter === 0 && learnedSpendResult?.learnedStateWritten === true && learnedSaveFixture.player?.talentEarning?.pointsAwarded === 1 && learnedSpendFixture?.player?.talentEarning?.pointsAwarded === 1 && learnedSpendFixture?.player?.talentLedger?.availablePoints === 0 && learnedSpendFixture?.player?.talentLedger?.spentPoints === 1 && learnedSpendFixture?.player?.talentLearnedIds?.hunter_board_clarity === true, JSON.stringify({ learnedSpendResult, learnedSaveFixture, learnedSpendFixture }));
    ...
    record('v1.21.2 build label is visible after reload', v12049Api?.build === '1.21.2' && String(v12049Api?.visibleLabel || '').includes('v1.21.2'), JSON.stringify({ build: v12049Api?.build, visibleLabel: v12049Api?.visibleLabel }));
    ...
    record('v1.21.2 spend button click succeeds in browser runtime', v12049SpendResult?.clicked === true && v12049SpendResult?.doubleClicked === true && v12049SpendResult?.ledger?.lifetimePoints === 1 && v12049SpendResult?.ledger?.availablePoints === 0 && v12049SpendResult?.ledger?.spentPoints === 1 && v12049SpendResult?.learned === true && Array.isArray(v12049SpendResult?.unlockIds) && v12049SpendResult.unlockIds.includes('hunter_board_clarity') && v12049SpendResult?.talents?.unlocked?.hunter_board_clarity === true && v12049SpendResult?.passive?.learned === true && v12049SpendResult?.passive?.passiveReady === true && v12049SpendResult?.passive?.passiveEnabled === true && v12049SpendResult?.passive?.appliesEffect === false && v12049SpendResult?.readiness?.enabled === false && v12049SpendResult?.readiness?.blockedReason === 'already_learned' && v12049SpendResult?.enabledSpendButtons === 0 && Array.isArray(v12049SpendResult?.spendButtons) && v12049SpendResult.spendButtons.length === 1 && v12049SpendResult.spendButtons[0].text === 'Spend 1 Talent Point' && v12049SpendResult.spendButtons[0].dataset.talentSpendHunterBoard === 'hunter_board_clarity' && v12049SpendResult?.spendButtons.every(btn => btn.disabled === true) && !v12049SpendResult?.buttonLabels.some(label => /^(Unlock|Purchase|Respec|Refund|Learn|Activate)$/i.test(label) && label !== 'Spend 1 Talent Point'), JSON.stringify(v12049SpendResult));

...
async function main() {
  ...
  try {
    ...
    const getLaneClarity = async () => await evalByValue(client, `(() => typeof revisitLaneStatusClarity === 'function' ? revisitLaneStatusClarity(S) : null)()`);
  ...
  const getBossTrophyState = async () => await evalByValue(client, `(() => ({
    ...
    const rulesetNodesOk = rulesetNodes.length === 13 && rulesetNodes.every(node => node.locked === true && node.previewOnly === true && node.active === false && node.gameplayEnabled === false && node.purchased === false && node.learned === false && node.unlocked === false && node.effectValue === 0 && node.applied === false);
    ...
    record('Readiness matrix reports every known passive contract', earningAudit?.readiness?.readinessSummary?.contractReady === 13 && earningAudit?.readiness?.readinessSummary?.total === 13, JSON.stringify(earningAudit?.readiness?.readinessSummary));
    ...
    record('Learned Debt Collector clarity is ready but guarded', earningAudit?.debtLearnedReadiness?.passivesReady?.find(entry => entry.id === TALENT_IDS.debtClarity)?.passiveReady === true && earningAudit?.debtLearnedReadiness?.passivesReady?.find(entry => entry.id === TALENT_IDS.debtClarity)?.passiveEnabled === false && earningAudit?.debtLearnedReadiness?.passivesReady?.find(entry => entry.id === TALENT_IDS.debtClarity)?.liveRendererWired === false, JSON.stringify(earningAudit?.debtLearnedReadiness));
    ...
    record('Activation gate dry run reports every known passive', !!hunterActivationGate && !!debtActivationGate && activationGateEntries.length === 13, JSON.stringify(activationGateEntries));
    ...
    record('Debt Collector activation gate keeps renderer guarded', debtActivationGate?.liveRendererWired === false && debtActivationGate?.passiveReady === false && debtActivationGate?.passiveEnabled === false && debtActivationGate?.canActivateNow === false && /not authorized/i.test(debtActivationGate?.activationBlockedReason || ''), JSON.stringify(debtActivationGate));
    ...
    record('Activation gate entries expose the complete stable metadata contract', Array.isArray(activationGateEntries) && activationGateEntries.every(entry => ['nodeKey', 'label', 'classification', 'readinessKnown', 'contractHelperPresent', 'displayCopyHelperPresent', 'smokeGuarded', 'learned', 'passiveReady', 'passiveEnabled', 'appliesCopyEffect', 'liveRendererWired', 'canActivateNow', 'activationBlockedReason', 'requiredFutureSteps', 'allowedSurface', 'blockedSystems', 'mutatesSave', 'appliesGameplayEffect'].every(field => Object.prototype.hasOwnProperty.call(entry, field))) && hunterActivationGate?.liveRendererWired === true && hunterActivationGate?.canActivateNow === false && debtActivationGate?.liveRendererWired === false && activationGateEntries.every(entry => entry.allowedSurface === 'display-copy only'), JSON.stringify(activationGateEntries));
    ...
    record('Learned debt_collector_clarity is preview-ready and renderer-guarded', earningAudit?.debtPassiveContractLearned?.nodeKey === TALENT_IDS.debtClarity && earningAudit?.debtPassiveContractLearned?.contractOwner === 'DungeonDexTalents' && earningAudit?.debtPassiveContractLearned?.learned === true && earningAudit?.debtPassiveContractLearned?.passiveReady === true && earningAudit?.debtPassiveContractLearned?.passiveEnabled === false && earningAudit?.debtPassiveContractLearned?.appliesEffect === false && earningAudit?.debtPassiveContractLearned?.liveRendererWired === false && earningAudit?.debtPassiveContractLearned?.guarded === true && earningAudit?.debtPassiveContractReplay?.passiveEnabled === false && JSON.stringify(earningAudit?.debtPassiveContractLearned) === JSON.stringify(earningAudit?.debtPassiveContractReplay), JSON.stringify(earningAudit?.debtPassiveContractLearned));
    ...
    record('Debt Collector alternate summary fixture reuses the same guarded copy helper', earningAudit?.debtAlternateApplied?.passiveSurface === 'Debt Collector copy-model preview only' && earningAudit?.debtAlternateApplied?.copyModelApplied === true && earningAudit?.debtAlternateApplied?.passiveApplied === false && earningAudit?.debtAlternateApplied?.liveRendererWired === false && earningAudit?.debtAlternateApplied?.statusLabel === 'Debt status: Owed marker' && earningAudit?.debtAlternateApplied?.balanceLabel === 'Amount owed: Owed 25 coin' && earningAudit?.debtAlternateApplied?.pressureLabel === 'Pressure: Pressure 4' && earningAudit?.debtAlternateApplied?.termsLabel === 'Terms: No delay.' && earningAudit?.debtAlternateApplied?.reminderLabel === 'Reminder: Pay soon.' && earningAudit?.debtAlternateApplied?.debtBalance === 2500 && earningAudit?.debtAlternateApplied?.debtPressure === 4 && earningAudit?.debtAlternateApplied?.repaymentState === 'pending' && earningAudit?.debtAlternateApplied?.saveStateToken === 'preserve-me-too' && earningAudit?.debtAlternateApplied?.combatState?.hp === 22 && earningAudit?.debtAlternateApplied?.rewardState?.coin === 7 && earningAudit?.debtAlternateApplied?.progressionState?.depth === 8, JSON.stringify(earningAudit?.debtAlternateApplied));
    ...
    record('v1.21.0 runtime exposes spend UI readiness helpers after reload', v12049Api?.hasApi === true && v12049Api?.hasApply === true && v12049Api?.hasMutation === true && v12049Api?.hasResultSummary === true && v12049Api?.hasUiReadiness === true && v12049Api?.hasHunterBoardUiReadiness === true, JSON.stringify(v12049Api));
    record('Current build label is visible after reload', v12049Api?.build === '1.23.4' && String(v12049Api?.visibleLabel || '').includes('v1.23.4'), JSON.stringify({ build: v12049Api?.build, visibleLabel: v12049Api?.visibleLabel }));
    record('v1.21.0 preview stays ready and read-only after reload', v12049Api?.preview?.nodeKey === 'hunter_board_clarity' && v12049Api?.preview?.eligible === true && v12049Api?.preview?.blockedReason === 'ready' && v12049Api?.preview?.liveSpendPatchReady === true && v12049Api?.preview?.requiresLiveSpendPatch === false && v12049Api?.preview?.mutatesSave === false && v12049Api?.preview?.spendsPoints === false && v12049Api?.preview?.learnsNode === false, JSON.stringify(v12049Api?.preview));
    ...
    record('v1.21.0 spend button is rendered once and only once for hunter_board_clarity', Array.isArray(v12049Api?.spendButtons) && v12049Api.spendButtons.length === 1 && v12049Api.spendButtons[0].text === 'Spend 1 Talent Point' && v12049Api.spendButtons[0].dataset.talentSpendHunterBoard === 'hunter_board_clarity' && v12049Api.spendButtons[0].disabled === false, JSON.stringify(v12049Api?.spendButtons));
    ...
    record('v1.21.2 duplicate spend blocks as already learned', v12049Duplicate?.ok === false && v12049Duplicate?.blockedReason === 'already_learned' && v12049AfterSpend?.save?.availablePoints === 0 && v12049AfterSpend?.save?.spentPoints === 1 && v12049AfterSpend?.learned === true, JSON.stringify(v12049Duplicate));
    await client.send('Page.reload', { ignoreCache: true });
    await waitForCondition(client, `!!window.DungeonDexScenarioDevTools && !!window.DungeonDexTalents && typeof render === 'function' && typeof S !== 'undefined' && !!S && !!S.player && document.body && document.readyState !== 'loading'`, 15000);
    ...
    record('Current build reload preserves spend state', v12049Reloaded?.build === '1.23.4' && String(v12049Reloaded?.visibleLabel || '').includes('v1.23.4') && v12049Reloaded?.learned === true && Array.isArray(v12049Reloaded?.unlockIds) && v12049Reloaded.unlockIds.includes('hunter_board_clarity') && v12049Reloaded?.save?.lifetimePoints === 1 && v12049Reloaded?.save?.availablePoints === 0 && v12049Reloaded?.save?.spentPoints === 1 && Object.prototype.hasOwnProperty.call(v12049Reloaded?.save || {}, 'awardClaims'), JSON.stringify(v12049Reloaded));
    record('v1.21.2 reload readiness model stays blocked on already learned', v12049Reloaded?.preview?.nodeKey === 'hunter_board_clarity' && v12049Reloaded?.readiness?.enabled === false && v12049Reloaded?.readiness?.blockedReason === 'already_learned' && v12049Reloaded?.readiness?.renderButtonNow === false && v12049Reloaded?.readiness?.clickHandlerEnabled === false && v12049Reloaded?.readiness?.uiActionWired === false, JSON.stringify(v12049Reloaded?.readiness));
    record('v1.21.2 reload keeps preview ready and forbids talent action buttons', v12049Reloaded?.preview?.nodeKey === 'hunter_board_clarity' && v12049Reloaded?.preview?.eligible === false && v12049Reloaded?.preview?.blockedReason === 'already_learned' && v12049Reloaded?.preview?.liveSpendPatchReady === true && v12049Reloaded?.preview?.requiresLiveSpendPatch === false && v12049Reloaded?.preview?.mutatesSave === false && v12049Reloaded?.preview?.spendsPoints === false && v12049Reloaded?.preview?.learnsNode === false && !v12049Reloaded?.hasForbiddenButton, JSON.stringify(v12049Reloaded?.preview));
    record('v1.21.2 reload UI contract stays learned-locked with no alternate spend target', v12049Reloaded?.learned === true && v12049Reloaded?.readiness?.enabled === false && v12049Reloaded?.readiness?.blockedReason === 'already_learned' && Array.isArray(v12049Reloaded?.spendButtons) && v12049Reloaded.spendButtons.length === 1 && v12049Reloaded.spendButtons[0].dataset.talentSpendHunterBoard === 'hunter_board_clarity' && v12049Reloaded.spendButtons[0].disabled === true && !v12049Reloaded.spendButtons.some(btn => btn.dataset.talentSpendHunterBoard && btn.dataset.talentSpendHunterBoard !== 'hunter_board_clarity') && v12049Reloaded?.preview?.nodeKey === 'hunter_board_clarity' && v12049Reloaded?.preview?.appliesEffect === false, JSON.stringify(v12049Reloaded));
    ...
    record('Canonical talent state contract helper exists', talentStateContractAudit?.hasHelper === true && talentStateContractAudit?.learnedDebt?.learned === true && talentStateContractAudit?.learnedDebt?.passiveReady === true && talentStateContractAudit?.learnedDebt?.passiveEnabled === false && talentStateContractAudit?.learnedDebt?.liveRendererWired === false && talentStateContractAudit?.learnedDebt?.guarded === true && talentStateContractAudit?.learnedDebt?.appliesEffect === false && talentStateContractAudit?.learnedDebt?.mutatesSave === false && talentStateContractAudit?.lockedDebt?.learned === false && talentStateContractAudit?.lockedDebt?.passiveReady === false && talentStateContractAudit?.lockedDebt?.passiveEnabled === false && talentStateContractAudit?.lockedDebt?.liveRendererWired === false && talentStateContractAudit?.lockedDebt?.appliesEffect === false && talentStateContractAudit?.lockedDebt?.mutatesSave === false && talentStateContractAudit?.previewNode?.previewOnly === true && talentStateContractAudit?.previewNode?.selectable === false && talentStateContractAudit?.previewNode?.selected === false && talentStateContractAudit?.previewNode?.learned === false && talentStateContractAudit?.previewNode?.passiveReady === false && talentStateContractAudit?.previewNode?.passiveEnabled === false && talentStateContractAudit?.previewNode?.appliesEffect === false && talentStateContractAudit?.previewNode?.liveRendererWired === false && talentStateContractAudit?.previewNode?.mutatesSave === false, JSON.stringify(talentStateContractAudit));
    ...
    const revisitLaneClarity = await getLaneClarity();
    ...
    const revisitTownActive = townRevisitText.includes('Trophy Echo') && (townRevisitText.includes('Start Trophy Echo') || townRevisitText.includes('Echo Locked') || townRevisitText.includes('Resolve Echo'));
    record('Revisit Town panel shows Trophy Echo lane', revisitTownActive && townRevisitText.includes('Famous Gear Memory') && !/Planned Return Routes|Earlier Dungeon Revisit/i.test(townRevisitText), townRevisitText.slice(0, 300));
    ...
    record('Route summary separates finished and unfinished preview lanes', revisitRouteSummary && Number.isInteger(revisitRouteSummary.total) && Number.isInteger(revisitRouteSummary.finished) && Number.isInteger(revisitRouteSummary.unfinished) && Array.isArray(revisitRouteSummary.finishedRoutes) && Array.isArray(revisitRouteSummary.unfinishedRoutes) && revisitRouteSummary.finished + revisitRouteSummary.unfinished >= revisitRouteSummary.total, JSON.stringify(revisitRouteSummary));
    record('Lane clarity helper distinguishes playable, finished, preview, planned, locked, and unknown states', Array.isArray(revisitLaneClarity) && revisitLaneClarity.some(lane => lane.key === 'board_echo_route' && lane.bucket === 'planned' && lane.shortLabel === 'Planned' && lane.isPlanned === true && lane.isLocked === true && lane.isPlayable === false && /board/i.test(lane.detailText || '') && /read-only|preview|activation/i.test(lane.nextStepText || '')) && revisitLaneClarity.some(lane => lane.key === 'debt_pressure_route' && lane.bucket === 'locked' && lane.shortLabel === 'Locked' && lane.isLocked === true && lane.isPlayable === false && /debt/i.test(lane.detailText || '') && /read-only|activation/i.test(lane.nextStepText || '')) && revisitLaneClarity.some(lane => lane.key === 'trophy_echo_route' && lane.bucket === 'finished' && lane.isFinished === true && lane.isPlayable === true), JSON.stringify(revisitLaneClarity));
    record('Revisit helper exposes playable Trophy Echo but keeps other lanes inert', revisitTownSource.includes('data-start-revisit="trophy_echo_route"') && revisitArchiveSource.includes('Famous Gear Memory remains inactive') && revisitMarketSource.includes('completeTrophyEchoRoute'), townRevisitText.slice(0, 300));
    const freshPanelText = await getTalentText();
    record('Talent panel renders release-candidate copy', freshPanelText.includes('Talent Tree Preview') && freshPanelText.includes('Locked preview only. Hunter Board Clarity is the only spendable path right now. The wider tree stays preview-only.') && freshPanelText.includes('Display-only copy clarity only. No stat, reward, combat, economy, or Revisit effects.') && freshPanelText.includes('Talent Points:') && freshPanelText.includes('First Talent source: Boss Trophy Milestone.') && freshPanelText.includes('Hunter Board Clarity: Elite Board contract wording only, not a stat bonus.') && freshPanelText.includes('No reward, risk, economy, combat, or Revisit values change.') && freshPanelText.includes('Learned:'), freshPanelText.slice(0, 500));
    record('Talent preview banner is visible', freshPanelText.includes('Locked preview') && freshPanelText.includes('Hunter Board Clarity is the only spendable path right now') && freshPanelText.includes('wider tree stays preview-only'), freshPanelText.slice(0, 300));
    record('Talent ledger copy is visible', freshPanelText.includes('Talent Ledger') && freshPanelText.includes('Only Hunter Board Clarity can spend now; the rest of the tree stays locked.') && freshPanelText.includes('0 available') && freshPanelText.includes('Spending inactive'), freshPanelText.slice(0, 400));
    ...
    record('Zero-point state shows preview locks', freshPanelText.includes('Locked preview only') && freshPanelText.includes('only spendable path') && !freshPanelText.includes('Need Point') && !freshPanelText.includes('Unlock Talent') && !freshPanelText.includes('Buy Talent'), freshPanelText.slice(0, 400));
    ...
    record('Repaired save preview display is safe', repairedPanelText.includes('Talent Tree Preview') && repairedPanelText.includes('Locked preview only. Hunter Board Clarity is the only spendable path right now. The wider tree stays preview-only.') && repairedPanelText.includes('Display-only copy clarity only. No stat, reward, combat, economy, or Revisit effects.'), repairedPanelText.slice(0, 400));
    record('Repaired save ledger display is safe', repairedPanelText.includes('Talent Ledger') && repairedPanelText.includes('Only Hunter Board Clarity can spend now; the rest of the tree stays locked.'), repairedPanelText.slice(0, 400));
    ...
    const bossEmptyText = await getBossTrophyText();
    {
      ...
      record('Retired Gear Hall archive shell appears', bossEmptyLower.includes('archive shelf') && bossEmptyLower.includes('retired gear records') && bossEmptyLower.includes('famous gear memory stays display-only') && bossEmptyLower.includes('trophy echo now lives in town'), bossEmptyText.slice(0, 500));
    }

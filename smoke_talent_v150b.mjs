...
async function main() {
  ...
  const getBossTrophyState = async () => await evalByValue(client, `(() => ({
    ...
    record('Talent panel renders release-candidate copy', freshPanelText.includes('Talent Tree Preview') && freshPanelText.includes('Locked preview only. No talent points, purchases, unlocks, or bonuses are active.') && freshPanelText.includes('Planned passives only. Inactive.') && freshPanelText.includes('Talent Points:') && freshPanelText.includes('First Talent source: Boss Trophy Milestone.') && freshPanelText.includes('Hunter Board Clarity: Elite Board contract wording only.') && freshPanelText.includes('No reward, risk, economy, or combat values change.') && freshPanelText.includes('Learned:'), freshPanelText.slice(0, 500));

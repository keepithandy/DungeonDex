...
The executable evidence for this report is `tests/smoke/smoke_rarity_progression_audit_v1280.mjs`. Runtime systems 04, 06, and 07 remain the balance authority. The audit calls real rarity/drop helpers, but its uniform set-collection model is conditional evidence, not a complete live-run simulation. Boss-source rows at non-boss depths (including D40 and D43) are hypothetical diagnostics, not available encounters. Boss cadence remains every 15 raw depths.

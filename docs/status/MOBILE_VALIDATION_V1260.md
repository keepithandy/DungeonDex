...
|---|---|---:|---|---|
| Shell and navigation | `#saveBtn`, `#resetBtn`, `[data-screen]`, `.ddx-nav-toggle`, `[data-town-shortcut]` | 44px; open routes 46px | Open drawer uses an 8px gap and scrolls inside the safe viewport. Holding Town toggles live Market, Forge, and Elite Contracts shortcuts. | Legacy toggle sizes were 24/30px and compact routes could fall to 31/34px. Final touch rules override them; the 768px Town title now clears the closed toggle. |
| Town entry, latest return, and charters | `#startRunBtn`, `#restBtn`, `[data-town-route]`, `[data-charter-start]` | 44px; narrow primary 52px and Rest 70px | Narrow Town actions stack without collisions; the latest-return receipt routes only to Gear or Journal, and Rest fields retain their own grid areas. | No remaining clipping or collision at 390, 430, or 768px. |
| Town economy, Scriptorium, and boards | `[data-debt-borrow]`, `#repayDebtBtn`, `#refreshMerchantBtn`, `[data-merchant-upgrade]`, `[data-spell-inscribe]`, `[data-spell-master]`, `[data-spell-respec]`, `[data-buy]`, `[data-buy-district]`, `[data-start-contract]`, `#forgeBtn`, `#salvageForgeBtn`, `[data-forge-slot]`, `[data-temper-slot]`, `#claimSparkWritBtn`, `#refreshSparkWritBtn`, `#claimEliteContractBtn`, `[data-start-revisit]`, `[data-complete-trophy-echo]`, `[data-guild-oath]`, `[data-guild-title]`, `[data-guildbound-sections]`, `button[disabled]` | 44px | `.ddx-action-row`, `.item-actions`, `.inline-actions`, and wrapped grids keep 8px separation where actions share a row. | Dynamic mini buttons previously inherited smaller desktop sizing; the final coarse-pointer rule covers every rendered instance. |
| Run and combat | `#runFromIdleBtn`, `[data-run-event]`, `[data-action]`, `#combatSpellbookBtn`, `[data-lantern-rite]`, `[data-lantern-skip]` | 44px; combat actions 46px | Attack, Ashburst, Guard, and Extract remain four equal in-flow columns with a 6px gap. The visible Spellbook trigger and Lantern draft choices stay outside the core action row. | The expanded spell and rite controls remain readable and do not cover combat text. |
| Gear filters, actions, inspection, and named loadouts | `#slotFilter`, `#rarityFilter`, `#sortFilter`, `#gearStatusFilter`, `#searchFilter`, `#sellJunkGearBtn`, `#sellAllGearBtn`, `[data-equip]`, `[data-sell]`, `[data-retire]`, `[data-clear-gear-filters]`, `[data-gear-flag]`, `[data-gear-detail-trigger]`, `#namedLoadoutName`, `[data-named-loadout-action]`, `[data-guildbound-preference]` | 44px | Item protection/junk controls, status filtering, bulk previews, filter reset, Escape-to-clear search, pointer and keyboard inspection, device-only display preferences, and loadout actions remain reachable and wrap cleanly. | Form controls, bulk actions, mini actions, role-buttons, preference toggles, and named-loadout actions are included by the final touch rule. |
| Modals, Archive, settings, and save transfer | `#introModalEnterDungeonBtn`, `#introModalContinueRunBtn`, `#introModalCloseBtn`, `[data-gear-detail-close]`, `[data-gear-detail-compare]`, `.trophy-tab`, `#clearCacheReloadBtn`, `#exportSaveBtn`, `#exportPreviousSaveBtn`, `#exportRecoverySaveBtn`, `#importSaveInput`, `#startFreshSaveBtn` | 44px | Modal bodies scroll inside the dynamic viewport; save backup/import controls remain reachable; unreadable saves expose recovery actions. | Mini close controls and save-transfer controls retain touch-safe sizing. |

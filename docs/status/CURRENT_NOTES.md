...
- Active continuation queue: `docs/status/ROADMAP_V128.md`. Local `main` includes the rarity audit, D31-D40 Reliquary slice, four-identity roster, and D45 conclusion with automated-browser verification. The no-change reward-curve decision now locks its live control points without tuning balance. See `RELIQUARY_LOADOUT_INTEGRATION.md`, `RELIQUARY_ROSTER_MILESTONE.md`, `RELIQUARY_D45_CONCLUSION.md`, and `REWARD_CURVE_DECISION_V128.md` for evidence and limitations. This source work is not physically device-validated. Historical branch version labels do not replace v1.28.2.

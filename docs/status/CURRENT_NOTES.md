...
- The owner confirms that the itch-ready v1.28.2 ZIP and accompanying devlog were uploaded to itch.io. The uploaded build contains the completed named source milestones through Journal/world reaction plus release-candidate hardening. Its original checksum and upload timestamp have not been independently recovered; the later local rebuild is not substituted as upload evidence. Physical-device/Textastic closure has started under `REAL_DEVICE_HANDOFF_V128.md`, but all 13 rows remain blocked until the owner records real-device evidence. No release tag is recorded.
- Active continuation queue: `docs/status/ROADMAP_NEXT_UPDATE.md`; `docs/status/ROADMAP_V128.md` is the completed v1.28 source-line plan. Local `main` includes the rarity audit, D31-D40 Reliquary slice, four-identity roster, D45 conclusion, themed gear presentation, loot/return clarity, Town/mobile polish, and Journal/world reaction with automated-browser verification. The no-change reward-curve decision locks its live control points without tuning balance; themed gear uses existing metadata only, while return and Town changes use existing records/routes and preserve actions, rewards, item mechanics, rolls, slots, loadouts, upgrades, and saves. See `RELIQUARY_LOADOUT_INTEGRATION.md`, `RELIQUARY_ROSTER_MILESTONE.md`, `RELIQUARY_D45_CONCLUSION.md`, `REWARD_CURVE_DECISION_V128.md`, `RELIQUARY_THEMED_GEAR.md`, and `LOOT_RETURN_TOWN_MOBILE_MILESTONES.md` for evidence and limitations. This source work is not physically device-validated. Historical branch version labels do not replace v1.28.2.
...
- The v1.28.2 version/cache authority remains active. The itch upload and devlog are owner-confirmed; no release tag is recorded. Continue from `ROADMAP_NEXT_UPDATE.md` without changing version/cache labels until separately authorized.
...
- The four proven combat actions retain their order, in-flow placement, 46px minimum height, and consistent 6px gap.
- Boss 3 onward now use a recoverable power ladder, replacing the old 3,600+ PWR Boss 3 spike. Over-scaled active boss saves repair their combat stats on reload while retaining current HP percentage and rewards.
- All 18 post-Boss-2 encounters pass the fully-upgraded district readiness audit. No normal-monster scaling, reward formulas, gear-upgrade values, progression, Revisit behavior, or dungeon entry changed.
- Build/cache labels use `1.26.4.04-boss-curve-release`.

## v1.26.4.01 Combat CSS Authority Playtest
- The existing four-column combat HUD and action layout now has one final CSS owner in `styles_visual_weight.css` instead of competing general and lore-layer declarations.
- The four proven combat actions retain their order, in-flow placement, and 46px minimum height, with a consistent 6px gap for play testing.
- Boss 3 onward now use a recoverable power ladder, replacing the old 3,600+ PWR Boss 3 spike. Over-scaled active boss saves repair their combat stats on reload while retaining current HP percentage and rewards.
- All 18 post-Boss-2 encounters pass the fully-upgraded district readiness audit. No normal-monster scaling, reward formulas, gear-upgrade values, progression, Revisit behavior, or dungeon entry changed.
- Build/cache labels use `1.26.4.01-combat-css-authority-playtest`.

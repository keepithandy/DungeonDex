...
- Active continuation queue: `docs/status/ROADMAP_V128.md`. Local `main` includes the rarity audit, D31-D40 Reliquary slice, four-identity roster, D45 conclusion, and themed gear presentation with automated-browser verification. The no-change reward-curve decision locks its live control points without tuning balance; themed gear uses existing metadata only and preserves item mechanics, rolls, slots, loadouts, upgrades, and saves. See `RELIQUARY_LOADOUT_INTEGRATION.md`, `RELIQUARY_ROSTER_MILESTONE.md`, `RELIQUARY_D45_CONCLUSION.md`, `REWARD_CURVE_DECISION_V128.md`, and `RELIQUARY_THEMED_GEAR.md` for evidence and limitations. This source work is not physically device-validated. Historical branch version labels do not replace v1.28.2.

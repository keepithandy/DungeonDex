...
// Item/monster/save normalization and persistence
  ...
  function normalizeMonster(monster, floor) {
    ...
    const postBossTwoExpectedPower = tier === 'Boss' && level > bossTwoDepth && typeof expectedMonsterPowerAtDepth === 'function'
      ? expectedMonsterPowerAtDepth(level, tier)
      : 0;
    const legacyPostBossTwo = tier === 'Boss' && level > bossTwoDepth && postBossTwoExpectedPower > 0 && power > postBossTwoExpectedPower * 1.3;
    const postBossTwoScale = legacyPostBossTwo && typeof postBossTwoPowerMultiplier === 'function'
      ? postBossTwoPowerMultiplier(level)
      : 1;
    const legacyBossRepair = legacyBossTwo || legacyPostBossTwo;
    const bossRepairScale = legacyBossTwo ? bossTwoScale : postBossTwoScale;
    const repairedMaxHp = legacyBossRepair ? Math.max(1, Math.round(maxHp * bossRepairScale)) : maxHp;
    ...
    return {
      ...
      level,
      power: legacyBossRepair ? Math.max(1, Math.round(power * bossRepairScale)) : power,
      maxHp: repairedMaxHp,
      hp: legacyBossRepair ? Math.max(1, Math.min(repairedMaxHp, Math.round(hp * bossRepairScale))) : hp,
      guard: legacyBossRepair ? Math.max(0, Math.round(guard * bossRepairScale)) : guard,
      speed: legacyBossRepair ? Math.max(0, Math.round(speed * bossRepairScale)) : speed,

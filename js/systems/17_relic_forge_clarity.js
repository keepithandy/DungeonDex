...
// DungeonDex v1.4.9a - Lowfire Forge clarity/lore overlay.
(function(){
  ...
  function forgePanelMarkup(state){
    ...
    return `<div class='card-head forge-head'>
      <div><h2>Lowfire Forge</h2><p>Lowfire breaks useless gear down, then hammers the pieces into equipment you can actually use.</p></div>
      <span class='pill rarity-rare'>Tier ${fmt(state.town.forgeTier)}</span>
    </div>
    <div class='tag-row forge-wallet'>
      <span class='pill'>Spark ${fmt(state.player.forgeSpark)}</span><span class='pill'>Shards ${fmt(state.player.shards)}</span><span class='pill'>Ember ${fmt(state.player.ember)}</span><span class='pill'>Forge Favor ${fmt(state.town.relicFavor)}</span>
    </div>
    <div class='sep'></div>
    <div class='forge-grid'><button class='primary' id='forgeBtn'>Forge Random Gear<br><span>1 spark + 40 shards</span></button><button class='ghost' id='salvageForgeBtn'>Salvage Junk<br><span>${safe} safe items</span></button></div>
    <p class='small muted forge-note'>Random Forge is the cheap option: spend a spark and shards to roll rare-or-better gear from any slot.</p>
    <div class='sep'></div>
    <p class='small forge-explain'><strong>Focused Forge:</strong> choose the gear slot before the item is made. The exact name, rarity, and stats still roll randomly. Costs 1 spark + 75 shards + 1 ember because you are forcing the shape of the gear.</p>

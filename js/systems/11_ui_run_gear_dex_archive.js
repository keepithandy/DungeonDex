...
// Run, gear, inventory, Dex, archive renderers
  ...
  function bossTrophyRecordCard(entry) {
    ...
    return `<article class="boss-trophy-record-card">
      ...
      <div class="boss-trophy-copy">
        ...
        <div class="boss-trophy-record-meta"><span class="pill trophy-pill-unlocked">Collected</span><span class="pill">Last Earned ${escapeHtml(lastEarned)}</span></div>
  ...
  function bossTrophyCard(trophy, bestDepth) {
    ...
    const lockedDescription = completion.key === 'available'
      ? 'Defeat this boss to add its trophy to the hall.'
      : 'Defeat deeper bosses to uncover this trophy.';
    return `<article class="boss-trophy-card ${unlocked ? 'is-unlocked' : 'is-locked'}">
      ...
      <div class="boss-trophy-copy">
        <div class="boss-trophy-card-topline">
          ...
          <span class="pill ${unlocked ? 'trophy-pill-unlocked' : 'trophy-pill-locked'}">${escapeHtml(unlocked ? 'Collected' : stateLabel)}</span>
        </div>
        <h3>${unlocked ? escapeHtml(trophy.name) : 'Unclaimed Boss Trophy'}</h3>

...
(function () {
  ...
  function journalMarkup(state) {
    ...
    return `<section class="guildbound-panel" aria-label="Crimson Oath chronicle"><div class="guildbound-heading"><div><span class="guildbound-kicker">Crimson Oath Chronicle</span><h3>Promises brought home</h3></div><span class="guildbound-chip">${escape(model.rank.name)}</span></div><p>${model.guild.renown} renown · ${model.totalCompleted} fulfilled ${model.totalCompleted === 1 ? 'oath' : 'oaths'} · ${model.guild.keepsakes.length} keepsakes</p><p>${model.guild.keepsakes.length ? `Keepsakes: ${model.guild.keepsakes.map(id => escape(keepsakeById(id).name)).join(' · ')}` : 'Prepare a Warden Oath in Lowfire, meet its goal, and extract alive to begin your Crimson Oath record.'}</p>${model.guild.selectedTitle ? `<p class="guild-oath-title">Title worn: ${escape(keepsakeById(model.guild.selectedTitle).title)}</p>` : ''}${history ? `<details><summary>Recent oath outcomes · ${model.guild.history.length}</summary><ul>${history}</ul></details>` : ''}</section>`;
  }

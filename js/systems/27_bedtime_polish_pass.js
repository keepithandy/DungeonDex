...
// DungeonDex v1.4.9b — Bedtime Polish Sweep.
// Broad low-risk UI/copy cleanup for Town, Lowfire Board, Relic Forge, talents, boss headers, and mobile spacing.
(function(){
  if (window.DDBedtimePolishPass) return;
  window.DDBedtimePolishPass = true;

  const BUILD = '1.4.9b';
  const LABEL = 'DungeonDex v' + BUILD;
  const BUILD_QS = '1.4.9b-bedtime-polish';
  ...
  function injectCss(){
    if (document.getElementById('ddBedtimePolishCss')) return;
    const style = document.createElement('style');
    style.id = 'ddBedtimePolishCss';
    style.textContent = `
  ...
  function wrap(name){
    ...
    if (typeof fn !== 'function' || fn.__bedtimePolished) return;
    const wrapped = function(){
      const result = fn.apply(this, arguments);
      run();
      return result;
    };
    wrapped.__bedtimePolished = true;

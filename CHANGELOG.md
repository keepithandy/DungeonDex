...
* Added a follow-up source-slot cache recovery pass using `1.25.2-revisit-source-slot` so the Revisit placement bridge is fetched fresh instead of the disabled no-op helper.
* Added an app-loader fallback that loads `44_revisit_lowfire_board_slot.js` with the source-slot cache key even if a local `index.html` copy still carries the old no-op query.

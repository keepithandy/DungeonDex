...
* Added a Revisit-only `Memory Mark` result that persists through save and reload without touching debt math, combat, gear stats, monster scaling, or broad Talent activation.
...
* Wired the claim repair contract into runtime loading and build/cache labels.
* Added save-shape repair for `player.talentLedger.awardClaims` so missing or malformed containers normalize to `{}` and only valid boss trophy milestone records survive.
* Kept points, live claim creation, spending UI, unlock UI, and gameplay mutation disabled.

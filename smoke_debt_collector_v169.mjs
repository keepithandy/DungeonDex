...
async function main() {
  ...
  try {
    ...
    record('Learned Debt panel keeps live renderer guarded', learnedPanelAudit?.html.includes('Active debt. Pressure is visible.') && learnedPanelAudit?.html.includes('Debt Active') && learnedPanelAudit?.html.includes('Owed 15s') && learnedPanelAudit?.html.includes('Pressure 3 • Rising') && learnedPanelAudit?.html.includes('Clarity: learned helper only') && learnedPanelAudit?.html.includes('Debt Collector Clarity is not active here') && learnedPanelAudit?.html.includes('data-debt-clarity-mode="copy-model"') && learnedPanelAudit?.html.includes('Status:</b> Debt Active') && learnedPanelAudit?.html.includes('Last Visit:</b> Test Visit') && learnedPanelAudit?.html.includes('Notes:</b> 1 note'), JSON.stringify(learnedPanelAudit));

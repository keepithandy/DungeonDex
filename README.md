...
Current baseline: **DungeonDex v1.28.1**
...
Current status: active flagship browser RPG. The live build includes the core dungeon loop, the D31-D40 Drowned Reliquary vertical slice, a clearer three-hunt Elite Contract Board, exact-target combat cues and contract records, safe-area-aware side rail navigation for desktop hover/focus and a compact 44px mobile/touch `←` drawer control, Lowfire Forge gear crafting/tempering, merchant gear upgrades, Guild Journal memory surfaces, Trophy Echo as the only active Revisit lane, a Gear-tab upgrade summary, custom header/Town visual identity assets, and pointer/keyboard-accessible gear detail modals.
...
- **Drowned Reliquary:** D31-D40 now has its own district identity, atmosphere, visual treatment, and two identity-only encounters while using the established combat, scaling, loot, and return pipelines.
...
- **Trophy Echo:** The only active Revisit lane for v1.28.1, tied to boss trophy or boss record history.
...
These systems may exist in old history or compatibility files, but they are not part of the active Revisit surface in v1.28.1:
...
Trophy Echo is the only active Revisit lane for v1.28.1. It stays small and deterministic: if the player has boss trophy history, town surfaces a short memory-reflection loop tied to that boss record.
